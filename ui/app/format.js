 ...
 */
export function bytes(value, locale = 'en') {
...
export function rate(bytesPerSecond, locale = 'en') {
    if (bytesPerSecond <= 0)
        return '—';
    return `${bytes(bytesPerSecond, locale)}/s`;
}
export function duration(seconds, locale = 'en') {
    if (seconds === null || !Number.isFinite(seconds) || seconds < 0)
        return '—';
    const n = (value) => Math.floor(value).toLocaleString(locale);
    if (seconds < 60)
        return `${n(seconds)}s`;
    if (seconds < 3600)
        return `${n(seconds / 60)}m ${n(seconds % 60)}s`;
    if (seconds < 86400)
        return `${n(seconds / 3600)}h ${n((seconds % 3600) / 60)}m`;
    return `${n(seconds / 86400)}d ${n((seconds % 86400) / 3600)}h`;
}
export function percent(done, total, locale = 'en') {
    if (!total || total <= 0)
        return '—';
...
export function timestamp(unixSeconds, locale = 'en') {
    if (!unixSeconds)
        return '—';
