...
import type { Download, Status } from './types.js';
...
const ICONS = {
  ...
  retry:
    '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M3 12a9 9 0 1 0 3-6.7M3 4v5h5"/></svg>',
} as const;

/** Actions offered for a download, which depend on what it is doing. */
export type RowAction = 'pause' | 'resume' | 'restart' | 'reveal' | 'remove';
...
export function renderList(
  ...
  let list = container.querySelector<HTMLElement>('.download-list');
  if (!list) {
    rows.clear();
...
function createRow(download: Download, callbacks: RowCallbacks): HTMLElement {
  ...
  row.addEventListener('click', (event) => {
    if (!(event.target as HTMLElement).closest('button')) open();
  });
...
function updateRow(row: HTMLElement, download: Download): void {
  const id = download.id;
  const name = download.filename || download.spec.url;
  const lang = locale();

  const badge = row.querySelector<HTMLElement>('.filetype')!;
  const ext = extension(name);
  if (badge.textContent !== ext) badge.textContent = ext;
  ...
  if (download.total) {
    parts.push(
      `<span>${bytes(download.downloaded, lang)} ${t('unit.of')} ${bytes(download.total, lang)}</span>`,
      `<span>${percent(download.downloaded, download.total, lang)}</span>`,
    );
  } else if (download.downloaded > 0) {
    parts.push(`<span>${bytes(download.downloaded, lang)}</span>`);
  }
  if (download.speed > 0) parts.push(`<span>${rate(download.speed, lang)}</span>`);
  ...
  renderBars(row.querySelector<HTMLElement>('.bars')!, download);
  renderActions(row.querySelector<HTMLElement>('.actions')!, download, id);
}
 ...
 */
function renderBars(container: HTMLElement, download: Download): void {
  const segments = download.segments ?? [];
  const useSegments = download.status === 'downloading' && segments.length > 1;

  if (useSegments) {
    let strip = container.querySelector<HTMLElement>('.segments');
    ...
    segments.forEach((segment, index) => {
      ...
      const fill = strip!.children[index]?.firstElementChild as HTMLElement | undefined;
      if (fill) fill.style.inlineSize = `${(fraction * 100).toFixed(1)}%`;
    });
    return;
  }

  let bar = container.querySelector<HTMLElement>('.progress');
  if (!bar) {
...
function progressClass(status: Status): string {
...
function renderActions(container: HTMLElement, download: Download, id: string): void {
  const buttons: { action: RowAction; icon: string; label: string; danger?: boolean }[] = [];
  ...
  container.replaceChildren(
    ...buttons.map(({ action, icon, label, danger }) => {
